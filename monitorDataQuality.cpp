...
int main(int argc, char *argv[]){
  ...
  ////...
  // Histograms for time of flight between plane pairs
  TH1F *h_timeOfFlight_TopBottom = new TH1F("h_timeOfFlight_TopBottom", Form("TimeOfFlight Top-Bottom, run%i", thisRun.GetRunNumber()), 40, -0.5, 55.5);
  h_timeOfFlight_TopBottom->GetXaxis()->SetTitle("Time of Flight [ns]");
  h_timeOfFlight_TopBottom->SetMinimum(0);
  h_timeOfFlight_TopBottom->SetLineColor(kRed);
  hist_list->Add(h_timeOfFlight_TopBottom);

  TH1F *h_timeOfFlight_UpstreamDownstream = new TH1F("h_timeOfFlight_UpstreamDownstream", Form("TimeOfFlight Upstream-Downstream, run%i", thisRun.GetRunNumber()), 40, -0.5, 55.5);
  h_timeOfFlight_UpstreamDownstream->GetXaxis()->SetTitle("Time of Flight [ns]");
  h_timeOfFlight_UpstreamDownstream->SetMinimum(0);
  h_timeOfFlight_UpstreamDownstream->SetLineColor(kBlue);
  hist_list->Add(h_timeOfFlight_UpstreamDownstream);

  TH1F *h_timeOfFlight_NorthSouth = new TH1F("h_timeOfFlight_NorthSouth", Form("TimeOfFlight North-South, run%i", thisRun.GetRunNumber()), 40, -0.5, 55.5);
  h_timeOfFlight_NorthSouth->GetXaxis()->SetTitle("Time of Flight [ns]");
  h_timeOfFlight_NorthSouth->SetMinimum(0);
  h_timeOfFlight_NorthSouth->SetLineColor(kGreen);
  hist_list->Add(h_timeOfFlight_NorthSouth);
////.....
  ...
  for (auto  eventit : thisRun.GetRunEventsList()) {
    ...
    //......................
    if (eventit.GetEventTimeOfFlight() != 0) {
      std::pair<int, int> planePair;
      std::map<std::string, int> PlaneNumbers;
    if (planePair == std::make_pair(PlaneNumbers["T"], PlaneNumbers["B"])) {
        h_timeOfFlight_TopBottom->Fill(eventit.GetEventTimeOfFlight());
    } else if (planePair == std::make_pair(PlaneNumbers["U"], PlaneNumbers["D"])) {
        h_timeOfFlight_UpstreamDownstream->Fill(eventit.GetEventTimeOfFlight());
    } else if (planePair == std::make_pair(PlaneNumbers["L"], PlaneNumbers["R"])) {
        h_timeOfFlight_NorthSouth->Fill(eventit.GetEventTimeOfFlight());
    }
    }
//...........................
    event_counter++;
 ...
 // Create a canvas and draw the ToF histograms for plane pairs...............
TCanvas *c_planePairsToF = new TCanvas("c_planePairsToF", "Time of Flight for Plane Pairs", 800, 600);

// Draw histograms with different colors
h_timeOfFlight_TopBottom->Draw("HIST");
h_timeOfFlight_UpstreamDownstream->Draw("HIST SAME");
h_timeOfFlight_NorthSouth->Draw("HIST SAME");
...
// Tạo một TCanvas để vẽ các histogram
TCanvas *c1 = new TCanvas("c1", "Data Quality Histograms", 800, 600);
c1->Divide(2, 2); // Chia canvas thành 2 hàng và 2 cột

// Vẽ các histogram lên các vùng của canvas
c1->cd(1); h_timeOfFlight_TopBottom->Draw();
c1->cd(2); h_timeOfFlight_UpstreamDownstream->Draw();
c1->cd(3); h_timeOfFlight_NorthSouth->Draw();
// Bạn có thể vẽ các histogram khác vào các vùng còn lại của canvas

// Hiển thị canvas
c1->Show();

// Khởi chạy vòng lặp của ứng dụng ROOT
app->Run();
/////
